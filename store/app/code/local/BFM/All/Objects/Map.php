 ...
 * BFM_All_Objects_Map can also be used like a regular array as follows,
 * <pre>
 ...
 * </pre>
 */
class BFM_All_Objects_Map extends BFM_All_Objects_Component implements IteratorAggregate, ArrayAccess, Countable
{
    /**
     ...
     * @throws BFM_All_Objects_Exception If data is not null and neither an array nor an iterator.
     */
    ...
    {
        return new BFM_All_Objects_MapIterator($this->_d);
    }
     ...
     * @throws BFM_All_Objects_Exception if the map is read-only
     */
    public function add($key, $value)
    {
        if (!$this->_r) {
            ...
            throw new BFM_All_Objects_Exception(Mage::helper('bfmall')->__('The map is read only.'));
    }
    ...
    public function copyFrom($data)
    {
        if (is_array($data) || $data instanceof Traversable) {
            if ($this->getCount() > 0)
                $this->clear();
            if ($data instanceof BFM_All_Objects_Map)
                $data = $data->_d;
            foreach ($data as $key => $value)
                $this->add($key, $value);
        }
        else if ($data !== null)
            throw new BFM_All_Objects_Exception(Mage::helper('bfmall')->__('Map data must be an array or an object implementing Traversable.'));
    }
