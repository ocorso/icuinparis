<?php
/*    
    Plugin Name: Pluginterest by Themeshock version 1.5 (Full Version)
                            ...
                            <table>                                
                                <tr>
                                    <td colspan="2" style="width: 400px;">
                                        <label for="boxwidth"><?php echo __("Box Width (in pixels, minimum Recommended 150):",'usefulmatrixint'); ?></label>
                                    </td>
                                    <td>
                                ...
                                <tr>
                                    <td colspan="2">
                                        <label for="pint_img">Url to Image when no images exists</label>
                                    </td>
                                    <td>
                                        <input type="text" value="<?php echo  plugin_dir_url(__FILE__) ?>img/pintimg.jpg" id="pint_img" name="pint_img" placeholder="http://" class="gen_shortcode " />
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan="2">
...
function wpts_useful_matrix_shortcode($atts=array()){
	...
	if(!isset($atts['pintimg']) || $atts['pintimg']==''){
        $atts['pintimg']=plugin_dir_url(__FILE__).'img/pintimg.jpg';
    }
...
//        $content1.= "<link rel='stylesheet' href='".plugin_dir_url(__FILE__)."css/bootstrap.min.css'  />";
	...
	if($categories!='none'){
		if($categories!='all'){			
			$get_cat=get_categories('include='.$categories);//get_categories($args);
			ob_start(); //this id for get categiories with descriptions
			?>
			<script>
				jQuery(document).ready(function(){
						if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box10')===true){
							jQuery.getJSON('http://api.wipmania.com/jsonp?callback=?', function (data) {
						    jQuery.getJSON("http://json-time.appspot.com/time.json?tz=UTC&callback=?",
						        function(data){
						        	if(data.minute<10){
						        		$('.hour').html(data.hour+ ':0' + data.minute);
						        	}else{
						    ...
						    /*iphone*/
					   		jQuery('.item<?php echo $sh_count;?>').each(function(){
					   			/*asdfasdfasdf*/
			            		jQuery(this).prepend('<div class="speaker"></div><br><div class="bar"><span class="domain">'+document.domain+'</span> <span class="wifi">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span><span class="hour"></span></div><br>');
								jQuery(this).append('<div class="minicontent" style="padding-top:5px;position:relative;top:-5px;">'+jQuery(this).html()+'</div>');
			            		jQuery(this).append('<div class="iphone-btn"></div>');
			            		jQuery(this).find('a:last').addClass('readmore');
			            		var btn=$(this).find('a:last');
			            		jQuery('.minicontent').find('.speaker,.bar:first,br').remove();
			            		jQuery(this).find('.contentTitle:first,.img_shadow:first,audio:first,video:first,p:first,.readmore:first,.pa:first').remove();
			            		//button style
			            		btn.addClass('btn').addClass('btn-default');
			            		/*conditions*/
					   		});
						}
						/*gothic*/
						if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box11')===true){
							jQuery('.item<?php echo $sh_count;?>').each(function(){
			            		//button style
			            		var btn=$(this).find('a:last');
			            		btn.addClass('btn').addClass('btn-default');
						...
						/*modern*/
						if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box12')===true){
							jQuery('.item<?php echo $sh_count;?>').each(function(){
			            		//button style
			            		var btn=$(this).find('a:last');
						...
						/*statusfive*/
						if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box13')===true){
							jQuery('.item<?php echo $sh_count;?>').each(function(){
			            		//button style
			            		var btn=$(this).find('a:last');
			            		btn.addClass('btn').addClass('btn-warning');
			            		/*conditions*/
					   		});
						}
						/*folk*/
						if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box14')===true){
							jQuery('.item<?php echo $sh_count;?>').each(function(){
			            		//button style
			            		var btn=$(this).find('a:last');
			            		btn.addClass('btn').addClass('btn-danger');
						...
						/*dragees*/
						if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box15')===true){
							jQuery('.item<?php echo $sh_count;?>').each(function(){
								if($(this).find('.contentTitle').html()!=null){
									$(this).prepend('<div class="horizontal-flag"><div class="contentTitle">'+jQuery(this).find('.contentTitle').html()+'</div></div>');
								}
								jQuery(this).find('.contentTitle:nth-child(2)').remove();
			            		//button style
			            		var btn=$(this).find('a:last');
			            		btn.addClass('btn').addClass('btn-info');
			            		/*conditions*/
					   		});
						}
						/*forest*/
						if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box16')===true){
							jQuery('.item<?php echo $sh_count;?>').each(function(){
								jQuery(this).append('<br><br><br><div class="branche"></div>');
			            		//button style
			            		var btn=$(this).find('a:last');
			            		btn.addClass('btn').addClass('btn-success');
					   		});
						}
						/*labyrinth*/
						if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box17')===true){
							jQuery('.item<?php echo $sh_count;?>').each(function(){
						...
						/*ics*/
						if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box9')===true){
							jQuery('.item<?php echo $sh_count;?>').each(function(){
			            		var btn=jQuery(this).find('a:last');
			            		//button style
				            	btn.addClass('btn').addClass('btn-info');
			...
			</script><!--end for draw categories listed by ID's with description without post-->
			<?php
			$script_pt_ts.=ob_get_clean();
			echo "<div id='wpts_container$sh_count' style='width: 100%;text-align:center;vertical-align: top;margin:auto;'>";
			...
			foreach($get_cat as $category){
				...
				if($showimage=="1"){
					...
					$cat_keys = array_keys($imgExtract);
				...
				<?php
					if($buttoncolor!='nobutton'){
		            	echo $button= "<div class='readmore'><a class='btn btn-small {$colors[$buttoncolor]}' href='".get_category_link( $category->term_id )."'>".$buttontext."</a></div>";                
		            }else{
		                echo $button= "<br /><a style='font-size: ".$contentsize."px;$contentf' href='".get_category_link( $category->term_id )."'>".$buttontext."</a>";
		            }
				echo "</div>";      
			}
		echo "</div>";
		}else{
			$get_cat=get_categories('orderby=name');//get_categories($args);
			ob_start(); //this id for get categiories with descriptions
			?>
			<script>
				jQuery(document).ready(function(){
			...
			</script><!--end for draw categories with descriptions-->
			<?php
			$script_pt_ts.=ob_get_clean();
			echo "<div id='wpts_container$sh_count' style='width: 100%;text-align:center;vertical-align: top;margin:auto;'>";
			$counter=0;///conteador de elementos
        ...
        //if(!is_wp_error($rss)){
            ...
            if($rssq>0){
                                                    ...
                                                    if (!isset($limit)){
                                                    	?>
                                   			<script>
                                   			//this is the version for the demo plugin into grdilayoutshock.com -- masonry is active
												$(document).ready(function(){
														if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box10')===true){
														    ...
														    /*iphone*/
													   		jQuery('.item<?php echo $sh_count;?>').each(function(){
													   			/*asdfasdfasdf*/
											            		jQuery(this).prepend('<div class="speaker"></div><br><div class="bar"><span class="domain">'+document.domain+'</span> <span class="wifi">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span><span class="hour"></span></div>');
																jQuery(this).append('<div class="minicontent" style="padding-top:5px;">'+jQuery(this).html()+'</div>');
											            		jQuery(this).append('<div class="iphone-btn"></div>');
											            		jQuery(this).find('a:last').addClass('readmore');
											            		var btn=jQuery(this).find('a:last');
											            		jQuery('.minicontent').find('.speaker,.bar:first,br').remove();
											            		jQuery(this).find('.contentTitle:first,.img_shadow:first,audio:first,video:first,p:first,.readmore:first').remove();
											            		//button style
											            		btn.addClass('btn').addClass('btn-default');
											            		/*conditions*/
													   		});
														...
														if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box15')===true){
															jQuery('.item<?php echo $sh_count;?>').each(function(){
																if(jQuery(this).find('.contentTitle').html()!=null){
																	jQuery(this).prepend('<div class="horizontal-flag"><div class="contentTitle">'+$(this).find('.contentTitle').html()+'</div></div>');
																}
																jQuery(this).find('.contentTitle:nth-child(2)').remove();
											            		//button style
											            		var btn=jQuery(this).find('a:last');
														...
														if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box16')===true){
															jQuery('.item<?php echo $sh_count;?>').each(function(){
																$(this).append('<br><br><br><div class="branche"></div>');
											            		//button style
											            		var btn=jQuery(this).find('a:last');
											            		btn.addClass('btn').addClass('btn-success');
													   		});
											...
											</script><!--end for Demonstrate version in gls.com -->
                                                    	<?php                               
                        $content1.= "\n<div id='wpts_container$sh_count' style='margin:auto;width: 100%;text-align:center;vertical-align: top;margin:auto;'>";
                                                    }else{
                                        ...
                                        $content1.= "\n<div id='wpts_container$sh_count".$col_st."' class='item ".$user_status."' style='width: 100%;margin:auto;'>";                               
					...
					if($boxstyle=='15'){
						if($showtitle=="1"){
	                        $content1.= "\n\t<div class='contentTitle' style='font-size: ".$titlesize."px;font-weight: bold;$titlef;padding: 5px'>".substr($item->get_title(), 0, 15)."...</div>";
	                    }
					}else{
	                    if($showtitle=="1"){
	                        $content1.= "\n\t<div class='contentTitle' style='font-size: ".$titlesize."px;font-weight: bold;$titlef;padding: 5px'>".$item->get_title()."</div>";
	                    }
					}
                    if($showimage=="1"){
                        $content1.= "$tmb";
                    ...
                    if($showexcerpt=="1"){
                        ...
                        $content1.= "<div class='pa' style='font-size: ".$contentsize."px;$contentf'><p>".$desc."</p></div>";
                    }
                    ...
                    if(isset($buttoncolor)){
                        ...
                        $content1.= "<div class='readmore'><br /><a class='btn btn-small {$colors[$buttoncolor]}' href='".$item->get_permalink()."'>".__($buttontext,'usefulmatrixint')."</a></div>";                
                            ...
                            $content1.= "<br /><a class='readmore' style='font-size: ".$contentsize."px;$contentf' href='".$item->get_permalink()."'>".__($buttontext,'usefulmatrixint')."</a>";
                        }
                    }else{
                        $content1.= "<br /><a class='readmore' style='font-size: ".$contentsize."px;$contentf' href='".$item->get_permalink()."'>".__($buttontext,'usefulmatrixint')."</a>";
                    }                    
...
function pint_make_html($get_cat,$category_asigned,$taxo_html,$q_posts,$sh_count,$pt_menu,$atts,$ajax_mode){
  ...
  ?>
  <div class="pt_main_drp pt_grp_<?php echo $sh_count; ?>" style="text-align: left; width: 100%; left: 5%; top: -8px;">
    ...
    <ul class="datasource">
      ...
      ?>
        ...
        switch ($category) {
         ...
         ?>
         <input type="checkbox" name="themeshock_categories" class="chk_categories" style="display: none;" value="<?php echo $data_name; ?>" id="<?php echo $data_name; ?>" data-base='false' data-menu=".pt_grp_<?php echo $sh_count; ?>" data-name="filter<?php echo $data_name; ?>" data-input="data_sh_<?php echo $sh_count; ?>" data-handler="<?php echo $data_handler;  ?>"   data-taxo="<?php echo $taxo_html; ?>" data-limit="<?php echo $data_limit;?>"/><label for="<?php echo $data_name; ?>"><li class="btn">
      	<?php echo $category_pack->name; ?>
      </li></label>
      <?php	
      ...
      ?>
      <!--<input type="checkbox" name="" id=""  data-menu=".pt_grp_<?php echo $sh_count; ?>" data-name="<?php echo $category_asigned; ?>" data-input="data_sh_<?php echo $sh_count; ?>" data-handler="<?php echo $data_handler;  ?>"   data-taxo="<?php echo $taxo_html; ?>" data-base='true' data-limit="<?php echo $data_limit;?>"/><li class="btn btn-primary">
     	 <a href="#">All <?php echo $category_asigned; ?></a>
      </li>-->
    </ul>
...
?>
	 ...
	 var ids;
	 jQuery('div.pt_grp_<?php echo $sh_count;?> ul.datasource input[type=checkbox]').on('change',function() {
	    ids = jQuery('input[type=checkbox]:checked').map(function() {
	        return '.' + jQuery(this).data('name');
	    }).get();
	    //ids.join(' ');
	    //alert('main height'+jQuery('#wpts_container<?php echo $sh_count;?>').height());
	    $(window).smartresize(function(){
	    	var newHeight=jQuery('#wpts_container<?php echo $sh_count;?>').height();
	    	//alert('main height'+jQuery('#wpts_container<?php echo $sh_count;?>').height());
	    	//alert(jQuery(window.parent.document).find('#get_slidershock').attr('id'));
	    	jQuery(window.parent.document).find('#get_slidershock').height(newHeight+100);
	    	//alert(jQuery(window.parent.document).find('#get_slidershock').height());
	    });
	 });
	 /*proof marker*/
	 $cont=jQuery('#wpts_container<?php echo $sh_count;?>');
				$(window).smartresize(function(){
				  $cont.find('.isotope-item').animate({
				  	top:0,
				  	left:0,
				  })
				});
			...
			/*var iframeHeight=jQuery('#get_slidershock').contents().find('#frame_he').find('#wpts_container1').height();
			jQuery('#get_slidershock').contents().find('#frame_he').find('.pt_grp_1').find('ul.datasource input[type=checkbox]').on('change',function(){
				var iframeHeightSet=jQuery('#get_slidershock').contents().find('#frame_he').find('#wpts_container1').height();
				alert(iframeHeight);
				alert(iframeHeightSet);
				jQuery('#get_slidershock').height(iframeHeightSet+100);
			});
			var iframeHeight=jQuery('#get_slidershock').contents().find('#frame_he').find('#wpts_container1').height();
			jQuery('#get_slidershock').contents().find('#frame_he').find('.pt_grp_1').find('ul.datasource input[type=checkbox]').on('change',function(){
      ...
      jQuery('div.pt_grp_<?php echo $sh_count;?> ul.datasource input[type=checkbox]').on('change',function(e) {
        //jQuery('div.pt_grp_<?php echo $sh_count;?> img.pint_loading').show();
        var cur_ele=jQuery(this);
        var checkedbox=jQuery(this).attr('id');
        jQuery('ul.datasource #'+ checkedbox + '+ label li').toggleClass('btn-inverse');
        ...
        var taxo=cur_ele.attr('data-taxo');
				...
				//alert(ids);
				/*pruebas para cambio de altura*/
				var iframeHeight;
				var parent=jQuery(window.parent.document).find('#get_slidershock');
				cont=jQuery('#wpts_container1');
				/*pruebas en cambio de altura*/
				$cont=jQuery('#wpts_container<?php echo $sh_count;?>');
				$cont.isotope({
				  filter : '.'+ids,
				...
				/* something */
				/*jQuery('#wpts_container1').on('resize',function(){
					alert('mi'+jQuery(this).height());
				})*/
				var iframeNewHeight=jQuery('#wpts_container1').height();
				var parentIframe=jQuery(window.parent.document).find('#get_slidershock').attr('id');
				jQuery('#'+parentIframe,top.document).height();
				jQuery(window.parent.document).find('#get_slidershock').height(iframeNewHeight+100);
				//newHeight=jQuery('#wpts_container1').height();
				
				//var iframeParent=$(window.parent.document).find('#get_slidershock').height();
				//alert(iframeNewHeight)
				//alert(iframeParent)
				/*tommorrow*/
				/*jQuery.ajax({
					url:'<?php echo get_bloginfo('wpurl');?>/wp-admin/admin-ajax.php',
					data:"action=pint_get_ctns&pinttg_taxo="+taxo+"&pinttg_category="+ids+"&pinttg_datalimit="+datalimit+'&pinttg_sht='+data_sht+'&pint_base='+data_base,
					type:'POST',
                    dataType:'json',
	...
	if ($ajax_mode===false){
		...
		<div id='wpts_container<?php echo $sh_count;?>' data-menu='pt_dr_menu_<?php echo $sh_count;?>' style='width: 100%;text-align: center;vertical-align: top;margin:auto;'>
				...
				jQuery(document).ready(function(){
						...
						if(jQuery('.item<?php echo $sh_count;?>').hasClass('useful_box10')===true){
						    ...
						    jQuery.getJSON("http://json-time.appspot.com/time.json?tz=UTC&callback=?",
						        function(data){
						        	if(data.minute<10){
						        		jQuery('.hour').html(data.hour+ ':0' + data.minute);
						        	}else{
						          		jQuery('.hour').html(data.hour+ ':' + data.minute);	
						        	}
						        })
						    });
						    /*iphone*/
...
$counter=0;
	...
	?>
				<div id='wpts_container<?php echo $sh_count.$col_st;?>.' class='item <?php echo $user_status; ?>' style='width: 100%;margin:auto;'>
	<?php
		...
		$output = preg_match('/<img[^>]+\>/i', $post->post_content, $matches);     
		       /*busqueda de los ids de la catgorias pertenecientes al post*/
		       //foreach((get_the_category()) as $fromcat) {echo ' filter'.$fromcat->cat_ID . ' ';} used for proof
		       /*Fin de la busqueda*/
	?>
    <div class="<?php echo "item$sh_count useful_box$boxstyle "; foreach((get_the_category()) as $fromcat) {echo 'filter'.$fromcat->cat_ID . ' ';} ?>" style="width:<?php echo $bwt;?>px;">
		<?php
		$bwt2=$bwt-10;
				...
				<a title='<?php the_title();?>' href='<?php the_permalink();?>'>
					...
					<img src='<?php echo $matches[1][0];?>' width='<?php echo $bwt2?>' height='<?php echo $bwt2 ?>' title='<?php the_title();?>' />
						<?php 
								...
								if($masonry=='1'){
									preg_match("/<img\s+.*?src=[\"\']?([^\"\' >]*)[\"\']?[^>]*>/i",get_the_content(),$m);
									?>
										<?php 
											if($m[0]==''){
												?>
													<img src='<?php echo $pintimg;?>' width='<?php echo $bwt2?>' height='auto' title='<?php the_title();?>' />
												<?php
											}else{
												echo $m[0];
											}
									...
									$contenttwitt=substr(strip_tags(get_the_content()),0,140);
									preg_match("/<img\s+.*?src=[\"\']?([^\"\' >]*)[\"\']?[^>]*>/i",$contenttwitt,$m);
									?>
										<?php echo $m[0];?>
									<?php 
								};
...
if($showtitle=="1"||empty($tmb)){
	...
	<?php 
      ...
      <a class="<?php echo $btn_shp_style; ?> shp_cart_button <?php echo $clas_btn; ?>"  style='font-size: <?php echo $contentsize."px;$contentf";?>'  tg_id_product="<?php the_ID(); ?>" tg_shp_type="tg_shp_add"  onclick="return false;" ondblclick="return false;" href="#"> 
		...
		?>
		 <a class="readmore" style='font-size: <?php echo $contentsize;?>px;<?php echo $contentf;?>' href='<?php the_permalink();?>'><?php echo __($buttontext,'usefulmatrixint')?></a>                 
				 <?php
		}else{
			?>
			<div class='readmore'>	
       ...
       <script type="text/javascript">
         ...
         <script type="text/javascript">
            if(typeof jQuery.fn.isotope === "undefined"){        
                document.write("<script type=\'text/javascript\' src=\'<?php echo plugin_dir_url(__FILE__)."js/isotope.js"; ?>\'></scr"+"ipt>");                
            }
        </script>
         <script type="text/javascript">
